...
/*!********************************************!*\
  !*** ./resources/js/admin/referPartner.js ***!
  \********************************************/
...
$(document).ready(function () {
  ...
  function listPage() {
    $.ajax({
      url: '/admin/refer/get-list-refer',
  ...
  function screen_list(data) {
    ...
    $.each(data.data['data'], function (index, referPartner) {
      ...
      html += '<button type="button" class="btn btn-sm btn-primary action_event me-1 mb-1 addUserToGroup"  data-name="' + referPartner.name + '" data-id="' + referPartner.id + '" data-bs-toggle="tooltip" data-bs-placement="top" title="Thêm khách hàng vào nhóm"><i class="bx bx-user-plus"></i></button>';
      html += '<a href="/admin/refer/edit/' + referPartner.id + '" class="btn btn-sm btn-warning text-light action_event me-1 mb-1" data-toggle="tooltip" data-placement="top" data-original-title="Chỉnh sửa"><i class="far fa-edit bx bx-edit"></i></a>';
      ...
      html += '<button type="button" class="btn btn-sm btn-danger action_event me-1 mb-1 delete-referPartner" data-action="delete" data-name="' + referPartner.name + '" data-id="' + referPartner.id + '" data-toggle="tooltip" data-placement="top" data-original-title="Xóa sự kiện"><i class="bx bx-trash"></i></button>';
      html += '</td>';
      html += '</tr>';
    });
    $('tbody').html(html);
    ...
    $('.delete-referPartner').on('click', function (e) {
      e.preventDefault();
      var referPartnerId = $(this).data('id');
      confirmDeleteReferPartner(referPartnerId);
    });
  }
  $(document).on('click', '.copy-refer-token', /*#__PURE__*/function () {
  ...
  function confirmDeleteReferPartner(referPartnerId) {
    var isConfirmed = confirm('Bạn có chắc muốn xoá nhóm khách tiếp thị liên kết này không??');
    if (isConfirmed) {
      $.ajax({
        url: "/admin/refer/delete/" + referPartnerId,
  ...
  $(document).on('click', '.addUserToGroup', function () {
    ...
    $('#modal-page .modal-title').text('Thêm người dùng vào nhóm tiếp thị liên kết');
    //$('#btn-submit').fadeIn().text('Thêm người dùng').attr('data-action', 'addUserToGroup').attr('data-id', id);
    $('#btn-submit-refer').fadeIn().attr('disabled', false);
    //$("#select-user-ids").val([]).trigger('change')
    var referPartnerId = $(this).data('id');
    $('#refer-id').val(referPartnerId);

    //console.log(id);
  });
  $('#btn-submit-refer').click(function (e) {
    e.preventDefault();
    ...
    $.ajax({
      ...
      type: "POST",
      url: "/admin/refer/add-user-to-group",
      dataType: "json",
      headers: {
      ...
      error: function error(_error3) {
        console.error("Thêm thất bại: ", _error3);
      }
    });
  });
