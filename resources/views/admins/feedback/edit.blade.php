            ...
            <form action="{{ route('admin.feedback.update', $feedback->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name ="order_id" class="form-select" id="order-select-clear-field" data-placeholder="Vui lòng chọn 1 đơn hàng" required>
                        ...
                        @foreach ($orders as $key => $orderName)
                            <option 
                                value="{{ $key ?? null }}"
                                {{ (collect(old('order_id'))->contains($key)) || $key == $feedback->order_id ? 'selected':'' }}
                            >
                                {{ $orderName }}
                            </option>
                        @endforeach
                ...
                <div class="mb-3">
                    ...
                    <textarea 
                        ...
                        required
                        rows="3"
                    >{!! old('content', $feedback->content ?? null) !!}</textarea>
                    @error('content')
                ...
                <div class="mb-3">
                    ...
                    <textarea 
                        ...
                        name="admin_note"
                        rows="3"
                    >{!! old('content', $feedback->admin_note ?? null) !!}</textarea>
                    @error('admin_note')
                ...
                <div class="mb-3">
                    ...
                    <select name ="stars" class="form-select" id="star-select-clear-field" data-placeholder="Vui lòng chọn đánh giá">
                        ...
                        @for ($i = 1; $i <= 5; $i++)
                            <option 
                                value="{{ $i }}"
                                {{ (collect(old('stars'))->contains($i)) || $i == $feedback->stars ? 'selected':'' }}
                            >
                                {{ $i }} sao
                            </option>
                        @endfor
                    ...
                    <select name ="status" class="form-select" id="status-select-clear-field" data-placeholder="Vui lòng chọn trạng thái" required>
                        ...
                        @foreach (\App\Enums\FeedbackStatus::FEEDBACK_STATUS as $feedbackStatus => $feedbackName)
                            <option 
                                ...
                                {{ (collect(old('status'))->contains($feedbackStatus)) || $feedbackStatus == $feedback->status ? 'selected':'' }}
                            >
                                {{ $feedbackName }}
                            </option>
                        @endforeach
                ...
                <div class="mb-3 form-group row">
                    ...
                    <div class="col-sm-6">
                        <div class="website-logo-container mb-2">
                            @if ( !empty($feedback->library->link) )
                                <img class="config-image" style="max-height: 300px; max-width: 300px; margin-left: 10px;" src="{{ asset($feedback->library->link)  }}" alt="{{ !empty($feedback->library->title) ? $feedback->library->title : null }}">
                            @else
                                <h6 class="text-danger">Chưa có hình ảnh cho feedback.</h6>
                            @endif
                        </div>
