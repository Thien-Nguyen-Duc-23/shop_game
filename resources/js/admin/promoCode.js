    ...
    function screen_list(data) {
        ...
        $.each(data.data['data'], function (index , promoCode) {
            ...
            if (parseInt(promoCode.type) == 1) {
                ...
                html += '<td>' + promoCode.value.toLocaleString('it-IT', {style : 'currency', currency : 'VND'}) +' </td>';
            }
            // Nội dung
            if (promoCode.order_min_value) {
                html += '<td>' + promoCode.order_min_value.toLocaleString('it-IT', {style : 'currency', currency : 'VND'}) + '</td>';
            } else {
                html += '<td> </td>';
            }
            if (promoCode.order_max_value) {
                html += '<td>' + promoCode.order_max_value.toLocaleString('it-IT', {style : 'currency', currency : 'VND'}) + '</td>';
            } else {
                html += '<td> </td>';
            }
